uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${AIRCRAFT_DATA_ROOT}" --data.dataset_name "FGVCAircraft" \
--data.kshot 16 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CARS_DATA_ROOT}" --data.dataset_name "StanfordCars" \
--data.kshot 16 --data.seed 10000


uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${CUB_DATA_ROOT}" --data.dataset_name "CUB-200-2011" \
--data.kshot 16 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${DTD_DATA_ROOT}" --data.dataset_name "DTD" \
--data.kshot 16 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${EUROSAT_DATA_ROOT}" --data.dataset_name "EuroSAT" \
--data.kshot 16 --data.seed 10000


uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FLOWERS_DATA_ROOT}" --data.dataset_name "Flowers102" \
--data.kshot 16 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${FOOD_DATA_ROOT}" --data.dataset_name "Food-101" \
--data.kshot 16 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${PET_DATA_ROOT}" --data.dataset_name "OxfordPets" \
--data.kshot 16 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 100

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1000

uv run tip_adapter.py --config configs/tip_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 100

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1000

uv run proto_adapter.py --config configs/proto_adapter.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 100

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1000

uv run ape.py --config configs/ape.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 100

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1000

uv run timo.py --config configs/timo.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 100

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1000

uv run timos.py --config configs/timos.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 1 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 2 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 4 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 8 --data.seed 10000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 100

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 1000

uv run protofuse.py --config configs/protofuse.yaml --model.backbone "ViT-L/14" \
--data.root "${UCF_DATA_ROOT}" --data.dataset_name "UCF101" \
--data.kshot 16 --data.seed 10000

