...
from src.pipelines.tip_adapter import TipAdapterPipeline

ARG_SCHEMA = DEFAULT_ARG_SCHEMA


def parse_args():
    parser = create_argument_parser("Run Tip-Adapter", ARG_SCHEMA)
...
def main():
    ...
    run_for_dataset_configs(merged, lambda dataset_config, _: TipAdapterPipeline(dataset_config).run())
