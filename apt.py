...
if __name__ == "__main__":
    ...
    parser.add_argument('--active_learning', type=str, default=None, choices=['entropy', 'random', 'coreset'], help='Active learning strategy to use (default: None)')
    ...
    parser.add_argument('--reset_optimizer_per_round', action='store_true', help='Reset optimizer and scheduler when adding new samples each round')
    ...
    selection_log_path = os.path.join(run_dir, 'al_selected_paths.log')
    ...
    cfg['al_selection_log'] = selection_log_path
    ...
    for round_idx in range(1, rounds + 1):
        ...
        strategy = args.active_learning
        round_selected_paths = []

        if strategy in ('entropy', 'random', 'coreset') and round_idx < rounds:
            if args.nshot > 0 and len(unlabeled_indices) > 0:
                if strategy == 'entropy':
                    entropy_scores = compute_entropy_scores(trainer, dataset, unlabeled_indices, batch_size, num_workers)
                    raw_selected = select_high_entropy_indices(entropy_scores, args.nshot)
                elif strategy == 'random':
                    raw_selected = select_random_indices(dataset, unlabeled_indices, args.nshot, seed=42 + round_idx)
                else:
                ...
                for idx in raw_selected:
                    if idx in unlabeled_indices and idx not in seen:
                        seen.add(idx)
                ...
                if selected_indices:
                    ...
                    if not new_indices:
                        ...
                        round_selected_paths = [os.path.abspath(dataset.samples[idx][0]) for idx in new_indices]
                        
                        selection_summary = f"Selected {len(new_indices)} new samples using {strategy} strategy."
                        print(selection_summary)
                        with open(log_file, 'a') as f:
                            f.write(selection_summary + '\n')
                            f.write(f"  Total labeled samples: {len(labeled_indices)} (was {len(labeled_indices) - len(new_indices)})\n")
                            f.write(f"  Total unlabeled samples: {len(unlabeled_indices)} (was {len(unlabeled_indices) + len(new_indices)})\n")
                        
                        if cfg.get('reset_optimizer_per_round', False):
                            reset_msg = "Resetting optimizer and scheduler for new samples"
                            print(reset_msg)
                    ...
                    al_msg = (
                        f"Active learning selection ({strategy}) (round {round_idx} -> {round_idx + 1}): no samples selected."
                    )
                    print(al_msg)
                    with open(log_file, 'a') as f:
                        f.write(al_msg + '\n')
                ...
                skip_msg = (
                    f"Active learning selection ({strategy}) (round {round_idx} -> {round_idx + 1}) skipped "
                    f"(nshot={args.nshot}, unlabeled={len(unlabeled_indices)})."
                )
                print(skip_msg)
                with open(log_file, 'a') as f:
                    f.write(skip_msg + '\n')

            with open(selection_log_path, 'a') as f:
                if round_selected_paths:
                    line = ';'.join(round_selected_paths)
                else:
                    line = f"round {round_idx}: none"
                f.write(line + '\n')
    ...
    cfg['samples_per_round'] = {
        'initial': cfg['initial_labeled_size'],
        'final': len(labeled_indices),
        'added_total': len(labeled_indices) - cfg['initial_labeled_size']
    }
    with open(config_path, 'w') as f:
        json.dump(cfg, f, indent=4)
