...
from apt import APT, APTTrainingPipeline
...
class ActiveLearningPipeline(APTTrainingPipeline):
    def __init__(self, config):
        ...
        self.kshot_budget = coerce_to_int(al_cfg.get('kshot_budget', self.kshot), self.kshot)
        self.num_rounds = coerce_to_int(al_cfg.get('num_rounds', 5), 5)

        base_output_value = self.logging_cfg.get("output_dir", "outputs/active_learning")
    ...
    def run(self):
        ...
        log_experiment_start("ActiveLearning+APT", dataset_name, self.kshot, self.seed)
        ...
        for round_idx in range(1, self.num_rounds + 1):
            logger.section(f"Active Learning Round {round_idx}/{self.num_rounds}", "train")
    ...
    def _run_al_round(self, round_idx, budget):
        ...
        logger.info(f"Round {round_idx}: labeled={len(self.labeled_indices)}, "
                     f"unlabeled={len(self.unlabeled_indices)}")
        ...
        for epoch_idx in range(1, epochs_total + 1):
            self._run_epoch(epoch_idx, epochs_total, train_loader, round_dir)
        ...
        selected_indices = []
        if len(self.unlabeled_indices) > 0 and round_idx < self.num_rounds:
            ...
            logger.info(f"Selected {len(selected_indices)} samples. "
                         f"New labeled={len(self.labeled_indices)}, "
                         f"unlabeled={len(self.unlabeled_indices)}")
        ...
        selection_record = {
            ...
            'selected_paths': [
                self.dataset.samples[i][0] for i in selected_indices
    ...
    def _select_samples(self, budget):
        actual_budget = min(budget, len(self.unlabeled_indices))

        unlabeled_ds = IndexTrackingDataset(self.dataset, self.unlabeled_indices)
        unlabeled_loader = DataLoader(
            unlabeled_ds, batch_size=self.batch_size * 4,
            shuffle=False, num_workers=self.num_workers,
        )

        logger.info(f"Scoring {len(self.unlabeled_indices)} unlabeled samples "
                     f"with {self.strategy.name} strategy...")

        score_result = self.strategy.score(
            self.trainer.model, unlabeled_loader, self.device,
        )
        local_scores = score_result['scores']

        global_scores = {
            self.unlabeled_indices[local_idx]: score
    ...
    def _move_to_labeled(self, selected_indices):
        ...
        self.unlabeled_indices = [
            i for i in self.unlabeled_indices if i not in selected_set
        ]
    ...
    def _finalize_al(self):
        ...
        logger.info(f"Rounds: {len(self.round_metrics)}, "
                     f"Final labeled: {len(self.labeled_indices)}")
...
def parse_args():
    parser = create_argument_parser("Train with Active Learning + APT", ARG_SCHEMA)
