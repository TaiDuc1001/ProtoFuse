...
CHECKPOINT_PATH = Path(__file__).parent.parent / "checkpoints" / "apt" / "d631caf1e5a72c24.pt"
...
def load_apt_model(checkpoint_path, classnames, device):
    ...
    model = CustomCLIP(cfg, classnames, clip_model, device)
...
def extract_apt_features(model, preprocess, class_to_images, selected_classes):
    ...
    print(f"Extracting APT text features for {len(selected_classes)} classes ({len(dataset)} images)...")
    
    all_features = []
    all_labels = []
    
    original_mode = model.cfg.get('mode', 'logits')
    model.cfg['mode'] = 'features'
    
    with torch.no_grad():
        for batch_images, batch_class_ids, batch_valid in tqdm(dataloader, desc="Extracting APT features"):
            batch_images = batch_images.to(DEVICE)
            
            _, text_features = model(batch_images)
            
            for i, (class_id, valid) in enumerate(zip(batch_class_ids, batch_valid)):
                if valid:
                    label_idx = class_id.item() - 1
                    feat = text_features[i, label_idx, :]
                    feat = feat / feat.norm()
                    all_features.append(feat.cpu().numpy())
                    all_labels.append(class_id.item())
    
    model.cfg['mode'] = original_mode
...
def plot_tsne(embeddings, labels, selected_classes, output_path, title="APT"):
...
def main():
    print("="*50)
    print("t-SNE: APT MODEL FEATURES")
    print("="*50)
    ...
    print("[2/6] Loading APT model from checkpoint...")
    model = load_apt_model(CHECKPOINT_PATH, classnames, DEVICE)
    ...
    print("[5/6] Extracting APT features...")
    features, labels = extract_apt_features(model, preprocess, class_to_images, selected_classes)
    print(f"       Extracted {len(features)} feature vectors")
    
    print("[6/6] Running t-SNE...")
    embeddings = run_tsne(features)
    
    output_path = OUTPUT_DIR / "tsne_apt.png"
    plot_tsne(embeddings, labels, selected_classes, output_path, title="APT ViT-B/16")
