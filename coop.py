...
class CoOPTrainingPipeline:
    ...
    def run(self):
        ...
        log_experiment_start("CoOP", dataset_name, self.kshot, self.seed)
    ...
    def _run_epoch(self, epoch_idx, epochs_total, train_loader, run_dir):
        ...
        if self.base_novel_enabled and base_val_acc is not None:
            logger.info(f"CoOP Epoch {epoch_idx} - loss={avg_loss:.4f} - acc={avg_acc:.2f}% - val_acc={val_acc:.2f}% - base={base_val_acc:.2f}% - novel={novel_val_acc:.2f}% - H={harmonic_mean:.2f}% - {epoch_time:.2f}s")
        else:
