...
class CheckpointCache:
    ...
    def get_checkpoint_path(self, checkpoint_id: str) -> str:
        return os.path.join(self.cache_dir, f"coop_ckpt_{checkpoint_id}.pt")
    ...
    def _update_index(self, checkpoint_id: str, key_settings: dict, path: str):
        ...
        fieldnames = ['checkpoint_id', 'file', 'method', 'dataset_root', 'kshot', 'seed',
                      'epochs', 'backbone', 'n_ctx', 'csc', 'created_at']
...
class CoOPTrainingPipeline:
    ...
    def _train_epochs(self):
        ...
        coop_dir = os.path.join(self.run_dir, 'coop')
        ...
        for epoch_idx in range(1, epochs_total + 1):
            self._run_epoch(epoch_idx, epochs_total, train_loader, coop_dir)

        self._save_checkpoint()

    def _run_epoch(self, epoch_idx, epochs_total, train_loader, coop_dir):
        if self.trainer is None:
        ...
        epoch_dir = os.path.join(coop_dir, f'epoch_{epoch_idx:03d}')
