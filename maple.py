...
class MaPLeTrainingPipeline:
    def __init__(self, config):
        ...
        base_output_value = self.logging_cfg.get("output_dir", "outputs/maple")
        base_output = coerce_to_str(base_output_value, "outputs/maple", key="logging.output_dir")
        ...
        self.trainer: Optional[MaPLe] = None
    ...
    def run(self):
        ...
        log_experiment_start("MaPLe", dataset_name, self.kshot, self.seed)
    ...
    def _initialize_trainer(self):
        ...
        self.trainer = MaPLe(self.trainer_cfg, self.classnames, device=str(self.device))
    ...
    def _run_epoch(self, epoch_idx, epochs_total, train_loader, run_dir):
        ...
        if self.base_novel_enabled and base_val_acc is not None:
            logger.info(f"MaPLe Epoch {epoch_idx} - loss={avg_loss:.4f} - acc={avg_acc:.2f}% - val_acc={val_acc:.2f}% - base={base_val_acc:.2f}% - novel={novel_val_acc:.2f}% - H={harmonic_mean:.2f}% - {epoch_time:.2f}s")
        else:
            val_acc_display = f"{val_acc:.2f}%" if self.val_loader is not None else "N/A"
            logger.info(f"MaPLe Epoch {epoch_idx} - loss={avg_loss:.4f} - acc={avg_acc:.2f}% - val_acc={val_acc_display} - {epoch_time:.2f}s")
